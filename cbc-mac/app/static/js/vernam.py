...
def validKey():
    ...
    # Do beautifulsoup or something to get the value of key
    key = "10101010"
    for i in key:
        if (i != "0") and (i != "1"):
            print("Only binary keys allowed!")
            return
    print("OK")
...
def Vernam_Encrypt():
    ...
    key = "1010111"
    if plaintext
    if (plaintext.length < 1) {alert("please enter some plaintext (binary only)")
                               return
                               }
    if (key.length < plaintext.length) {alert("key must be atleast the length of plaintext")
                                        return
                                        }
    ciphertext = ""
    for (i=0
...
function Vernam_Decrypt(f) {
    ...
    key = document.getElementById("key").value
    if (ciphertext.length < 1) {alert("please enter some ciphertext (binary only)")
                                return
                                }
    if (key.length < ciphertext.length) {alert("key must be atleast the length of ciphertext")
                                         return
                                         }
    plaintext = ""
    for (i=0
...
function next_plain_text() {
    document.getElementById(
        "plainarea").value = Vernam_RandSequence(PLAIN_TEXT_LEN)
}
...
function check_key() {
    ...
    if (zero == one) {
        document.getElementById(
            "key_notify").value = "Correct Key! You may proceed."
    } else {
...
function Encrypt_p() {
    ...
    document.getElementById("cipherarea").value = encrypt(
        document.getElementById("plainarea").value, k)
}
...
function Next_Vernam_Test() {
    ...
    key_len = (Math.random() * 100000) % 7 + 6
    document.getElementById(
        "keyarea_gen").value = Vernam_RandSequence(key_len)
    Next_Encryption()
}
...
function checkAnswer() {
    if (document.getElementById("yesno").value == 'yes') {
        document.getElementById(
            "notification").value = "This is not correct, Please try again!"
        return
    }
